...
try {
    $smsService = new NotificationService($conn);
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $action = $_POST['action'] ?? '';

        if ($action === 'fetch_all_contacts') {
            // Fetch all active numbers
            $query = "SELECT MobileNumber FROM tblemployees WHERE Status = 'Active' AND MobileNumber IS NOT NULL AND MobileNumber != ''";
        ...
        elseif ($action === 'search_members') {
            ...
            $query = "SELECT CoopID, CONCAT(FirstName, ' ', LastName) as fullname, MobileNumber 
                      FROM tblemployees 
                      WHERE (FirstName LIKE :term OR LastName LIKE :term OR CoopID LIKE :term) 
                      AND Status = 'Active' AND MobileNumber IS NOT NULL AND MobileNumber != ''
                      LIMIT 20";
