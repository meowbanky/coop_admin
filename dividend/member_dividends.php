...
<head>
    ...
    <title>Member Dividend Calculator</title>
...
<div class="container mx-auto p-6">
    <h1 class="text-3xl font-bold mb-6 text-center text-gray-800">Member Dividend Calculator</h1>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        <div class="grid grid-cols-1 gap-4">
            <div>
                <label for="member_names" class="block text-sm font-medium text-gray-700">Member Names (comma-separated)</label>
                <input type="text" id="member_names" name="member_names" placeholder="e.g., John Doe, Jane Smith" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            </div>
        </div>
        <button id="calculate" class="mt-4 w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50">Calculate Member Dividends</button>
    </div>

    <div id="results" class="bg-white p-6 rounded-lg shadow-md hidden">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-semibold text-gray-800">Member Dividend Results (Jan-Dec 2024)</h2>
        ...
        <div class="overflow-x-auto">
            <table id="memberDividendTable" class="min-w-full divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Dividend</th>
...
<script>
    $(document).ready(function() {
        ...
        function exportToExcel() {
            const table = document.getElementById('memberDividendTable');
            ...
            link.setAttribute('download', 'member_dividend_results.csv');
        ...
        $('#calculate').click(function() {
            const memberNames = $('#member_names').val().trim();
            ...
            $.ajax({
                url: 'calculate_member_dividends.php',
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        $.each(response.data, function(index, row) {
                            ...
                            const totalDividend = savings + shares + interest;

                            html += `
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">${row.COOPID}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">${row.FullName}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">${row.LastPeriod}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">${row.CoveragePercentage}%</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">${formatNumber(totalDividend.toFixed(2))}</td>
                ...
                error: function() {
                    hideLoading();
                    alert('An error occurred while calculating member dividends');
                }
            });
        });
        ...
        $('#exportExcel').click(function() {
            if ($('#results').hasClass('hidden')) {
                alert('Please calculate member dividends first to export data.');
                return;
            }
            exportToExcel();
        });
