...
try {
    ...
    // Try multiple methods to get Authorization header (POST requests sometimes need special handling)
    $authHeader = '';
    ...
    // Method 3: Fallback to $_SERVER (most reliable for POST requests)
    if (empty($authHeader)) {
    ...
    if (empty($authHeader) || !preg_match('/Bearer\s+(.*)$/i', $authHeader, $matches)) {
        error_log("Transaction Summary - No auth header found. Headers: " . print_r(getallheaders() ?? [], true));
        error_log("Transaction Summary - SERVER vars: " . print_r([
